...
import type { VoteSession, PagedResponse } from '../../api/types';
import { useTheme, useThemedStyles } from '../../contexts/ThemeContext';
import type { Theme } from '../../ui/theme';
import { Badge, DataList } from '../../components/ui';
import { useRefreshControl } from '../../hooks/useRefreshControl';

export function VotingListScreen({ navigation }: any) {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['votes'],
    queryFn: () => api.get<PagedResponse<VoteSession>>('/api/v1/meetings/all/votes'),
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <DataList
        ...
        renderItem={({ item }) => (
          <Pressable style={styles.item} onPress={() => navigation.navigate('VotingDetail', { id: item.id })}>
            <View style={{ flex: 1 }}>
              <Text style={styles.name}>{isAr ? item.titleAr : item.titleEn}</Text>
              <Text style={styles.meta}>{t('voting.totalVotes')}: {item.totalVotes ?? 0}</Text>
            </View>
            <Badge variant={item.status === 'open' ? 'success' : item.status === 'closed' ? 'info' : 'default'} label={t(`voting.statuses.${item.status}`, item.status)} size="sm" />
          </Pressable>
        )}
      />
    </View>
